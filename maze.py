...
class Maze:
    ...
    def __solve_r(self, i, j):
        ...
        for direction in directions:
            if direction == "right" and i + 1 < self.__num_cols and self.__cells[i+1][j].visited == False and self.__cells[i][j].has_right_wall == False:
                self.__cells[i][j].draw_move(self.__cells[i+1][j])
                if self.__solve_r(i+1, j) == True:
                    return True
                self.__cells[i][j].draw_move(self.__cells[i+1][j], True)
            elif direction == "left" and i - 1 > 0 and self.__cells[i-1][j].visited == False and self.__cells[i][j].has_left_wall == False:
                self.__cells[i][j].draw_move(self.__cells[i-1][j])
                if self.__solve_r(i-1, j) == True:
                    return True
                self.__cells[i][j].draw_move(self.__cells[i-1][j], True)
            elif direction == "top" and j - 1 > 0 and self.__cells[i][j-1].visited == False and self.__cells[i][j].has_top_wall == False:
                self.__cells[i][j].draw_move(self.__cells[i][j-1])
                if self.__solve_r(i, j-1) == True:
                    return True
                self.__cells[i][j].draw_move(self.__cells[i][j-1], True)
            elif direction == "bottom" and j + 1 < self.__num_rows and self.__cells[i][j+1].visited == False and self.__cells[i][j].has_bottom_wall == False:
                self.__cells[i][j].draw_move(self.__cells[i][j+1])
                if self.__solve_r(i, j+1) == True:
                    return True
                self.__cells[i][j].draw_move(self.__cells[i][j+1], True)
